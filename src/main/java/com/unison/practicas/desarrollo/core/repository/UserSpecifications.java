...
public interface UserSpecifications {
    ...
    static Specification<User> withActiveBookLoans(Integer min, Integer max) {
        return (root, query, cb) -> {
            ...
            var subRoot = sub.from(BookLoan.class);
            sub.select(cb.count(subRoot));
            sub.where(
                    cb.equal(subRoot.get("user"), root),
                    cb.isNull(subRoot.get("returnDate"))
            );

            if (min != null && max != null) {
                return cb.between(sub, min.longValue(), max.longValue());
            }

            if (min != null) {
                return cb.ge(sub, min.longValue());
            }

            return cb.le(sub, max.longValue());
        };
    }
