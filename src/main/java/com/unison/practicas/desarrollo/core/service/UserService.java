package com.unison.practicas.desarrollo.core.service;

import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.unison.practicas.desarrollo.core.dto.*;
import com.unison.practicas.desarrollo.core.util.PaginationRequest;
import com.unison.practicas.desarrollo.core.util.SortRequest;
import org.jooq.*;
import org.jooq.impl.DSL;
import org.springframework.stereotype.Service;
import org.springframework.util.CollectionUtils;

import java.time.OffsetDateTime;
import java.time.format.DateTimeFormatter;
import java.util.*;
import java.util.Comparator;

import static com.unison.practicas.desarrollo.jooq.Tables.*;

@Service
public class UserService {

    private static final Set<String> ALLOWED_SORTS = Set.of(
            "firstName",
            "lastName",
            "email",
            "phoneNumber",
            "createdAt",
            "updatedAt",
            "role"
    );

    private static final List<SortRequest> DEFAULT_SORTS = List.of(
            new SortRequest("lastName", "asc"),
            new SortRequest("firstName", "asc")
    );

    private final DSLContext dsl;
    private final DateTimeFormatter dateTimeFormatter;
    private final ObjectMapper objectMapper;

    public UserService(DSLContext dsl, ObjectMapper objectMapper) {
        this.dsl = dsl;
        this.objectMapper = objectMapper;
        this.dateTimeFormatter = createDateTimeFormatter();
    }

    public List<UserPreview> getUsersPreviews(UserPreviewsQuery query, PaginationRequest paginationRequest) {

        // ---- Active loans subquery ----
        Field<Integer> activeLoans = DSL.selectCount()
                .from(BOOK_LOAN)
                .where(BOOK_LOAN.USER_ID.eq(APP_USER.ID))
                .and(BOOK_LOAN.RETURN_DATE.isNull())
                .asField("active_loans");


        // Aggregate user roles as a json
        Field<JSON> rolesJson = DSL.jsonArrayAgg(
                DSL.jsonObject(
                        DSL.jsonEntry("id", APP_ROLE.ID),
                        DSL.jsonEntry("name", APP_ROLE.NAME),
                        DSL.jsonEntry("slug", APP_ROLE.SLUG),
                        DSL.jsonEntry("createdAt", APP_ROLE.CREATED_AT),
                        DSL.jsonEntry("updatedAt", APP_ROLE.UPDATED_AT)
                )
        ).as("roles_json");

        // ---- Roles names concatenation (sorted) ----
        Field<String> roleNamesConcat = DSL.groupConcat(APP_ROLE.NAME)
                .orderBy(APP_ROLE.NAME.asc())
                .separator(",")
                .as("roles_concat");

        // ---- Base query ----
        var base = dsl.select(
                        APP_USER.ID,
                        APP_USER.FIRST_NAME,
                        APP_USER.LAST_NAME,
                        APP_USER.EMAIL,
                        APP_USER.PHONE_NUMBER,
                        APP_USER.CREATED_AT,
                        APP_USER.UPDATED_AT,
                        activeLoans,
                        rolesJson,
                        roleNamesConcat
                )
                .from(APP_USER)
                .join(USER_ROLE).on(APP_USER.ID.eq(USER_ROLE.USER_ID))
                .join(APP_ROLE).on(APP_ROLE.ID.eq(USER_ROLE.ROLE_ID));

        // ---- Filters ----
        if (query.search() != null && !query.search().isBlank()) {
            String pattern = "%" + query.search().toLowerCase() + "%";
            base.where(DSL.lower(APP_USER.FIRST_NAME).like(pattern)
                    .or(DSL.lower(APP_USER.LAST_NAME).like(pattern))
                    .or(DSL.lower(APP_USER.EMAIL).like(pattern))
                    .or(DSL.lower(APP_USER.PHONE_NUMBER).like(pattern)));
        }

        if (!CollectionUtils.isEmpty(query.role())) {
            base.where(APP_ROLE.SLUG.in(query.role()));
        }

        if (query.memberSinceMin() != null) {
            base.where(APP_USER.CREATED_AT.ge(OffsetDateTime.from(query.memberSinceMin().atStartOfDay())));
        }

        if (query.memberSinceMax() != null) {
            base.where(APP_USER.CREATED_AT.le(OffsetDateTime.from(query.memberSinceMax().atTime(23, 59, 59))));
        }

        if (query.activeBookLoansMin() != null) {
            base.where(activeLoans.ge(query.activeBookLoansMin()));
        }

        if (query.activeBookLoansMax() != null) {
            base.where(activeLoans.le(query.activeBookLoansMax()));
        }

        // ---- Group by user for JSON aggregation ----
        base.groupBy(
                APP_USER.ID,
                APP_USER.FIRST_NAME,
                APP_USER.LAST_NAME,
                APP_USER.EMAIL,
                APP_USER.PHONE_NUMBER,
                APP_USER.CREATED_AT,
                APP_USER.UPDATED_AT,
                activeLoans
        );

        // ---- Sorting ----
        List<SortRequest> sorts = CollectionUtils.isEmpty(paginationRequest.sorts())
                ? DEFAULT_SORTS
                : paginationRequest.sorts();

        for (SortRequest sortReq : sorts) {
            if (!ALLOWED_SORTS.contains(sortReq.sort())) continue;

            if ("role".equals(sortReq.sort())) {
                base.orderBy("desc".equalsIgnoreCase(sortReq.order())
                        ? roleNamesConcat.desc()
                        : roleNamesConcat.asc());
            } else {
                base.orderBy(orderField(sortReq));
            }
        }

        // ---- Pagination ----
        int offset = paginationRequest.page() * paginationRequest.size();
        base.offset(offset).limit(paginationRequest.size());

        // ---- Execute ----
        var result = base.fetch();

        // ---- Map results ----
        return result.stream().map(r -> new UserPreview(
                r.get(APP_USER.ID).toString(),
                formatInvertedName(r.get(APP_USER.FIRST_NAME), r.get(APP_USER.LAST_NAME)),
                r.get(APP_USER.EMAIL),
                r.get(APP_USER.PHONE_NUMBER),
                parseRolesJson(r.get("roles_json", String.class)),
                dateTimeFormatter.format(r.get(APP_USER.CREATED_AT)),
                String.valueOf(r.get(activeLoans))
        )).toList();
    }

    private List<RoleResponse> parseRolesJson(String rolesJson) {
        try {
            List<Map<String, Object>> rolesList = objectMapper.readValue(
                    rolesJson,
                    new TypeReference<>() {}
            );

            return rolesList.stream()
                    .map(this::toRoleResponse)
                    .sorted(Comparator.comparing(RoleResponse::name))
                    .toList();

        } catch (Exception e) {
            throw new RuntimeException("Could not read user roles correctly");
        }
    }

    private RoleResponse toRoleResponse(Map<String, Object> map) {
        return new RoleResponse(
                map.get("id").toString(),
                map.get("name").toString(),
                map.get("slug").toString(),
                map.get("createdAt").toString(),
                map.get("updatedAt").toString()
        );
    }

    private org.jooq.SortField<?> orderField(SortRequest sortReq) {
        Field<?> field = switch (sortReq.sort()) {
            case "firstName" -> APP_USER.FIRST_NAME;
            case "email" -> APP_USER.EMAIL;
            case "phoneNumber" -> APP_USER.PHONE_NUMBER;
            case "createdAt" -> APP_USER.CREATED_AT;
            case "updatedAt" -> APP_USER.UPDATED_AT;
            default -> APP_USER.LAST_NAME;
        };
        return "desc".equalsIgnoreCase(sortReq.order()) ? field.desc() : field.asc();
    }

    private String formatInvertedName(String firstName, String lastName) {
        return "%s, %s".formatted(lastName, firstName);
    }

    private DateTimeFormatter createDateTimeFormatter() {
        return DateTimeFormatter.ofPattern(
            "dd/MMM/yyyy",
            new Locale.Builder()
                .setLanguage("es")
                .setRegion("MX")
                .build()
        );
    }
}